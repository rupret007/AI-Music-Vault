...
6. Dashboard session resume is convenience state, not the control-room handoff: it stores only schema version + work kind + catalog ID in this browser. **Click-test:** open the local dashboard → Click Write, Produce, or Listen → note song and `N of M` → Next if you want a non-first song → Refresh (exact song must reopen; do not land on the first match and overwrite) → Escape / Clear filters → **Resume** the same song → **Forget** (status: forgot; blocked storage soft-fails) → Refresh again (Resume stays gone). Extra fields, stale schema, unknown id, or a mismatched kind are rejected and removed. The latest Session Log remains authoritative.
