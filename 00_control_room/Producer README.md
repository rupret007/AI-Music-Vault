...
*How this system works, and how a future session resumes. Last updated: 2026-09-25 (usable catalog: local app_api.json after StoryBoard #20, spine rejected as an import, not a remote URL. Show Night official set is owner-only. Official-set dump binds as Rad Dad — official set. Show Night is the live set surface. Catalog rows are not the official set. Show Night owns official sets. Derived master_catalog.csv must match the spine. Song-level Logic-ready discovery is a control-room walk, not a second catalog; the private dashboard names a sanitized Logic-ready next in Logic Pro's own vocabulary from that existing evidence, plus a per-song Assets on file readout (Logic Pro project / WAV/AIFF / Stems / Recorded take / Key on file) computed from those same signals — a chain-only take never gets relabeled WAV/AIFF. Cover-book leftover is the 93-cover walk (Trailer Swift originals stay 0; official-set originals stay 0). Private dashboard find-and-act on searchable matched memos; Songs search finds existing aliases, folds punctuation, ranks closest name first, and Enter opens that row; write/produce/listen starts on one highest-momentum match with bounded progress and collapsed secondary filters; sit-down / session / Resume show the sanitized catalog next step plus Logic-ready next and its assets; Copy next step and Copy Logic-ready next fail closed; browser-local resume stores only work kind + catalog ID, refresh reopens the exact validated song, and Forget soft-fails; Logic Pro projects, keys, and WAVs stay owner-only. Session Log #12 heading listed once. Resume from the latest Session Log H2, not Session 1. Local validate is the catalog gate; empty-runner red was a private-repo billing-era symptom and is not a catalog fail by itself. The repo is now public for Actions, so hosted runs should execute normally. Jeff-facing README/APPS.md are roles and counts only. Catalog-gate success output is roles and counts only).*
...
6. Dashboard session resume is convenience state, not the control-room handoff: it stores only schema version + work kind + catalog ID in this browser. **Click-test:** open the local dashboard → Click Write, Produce, or Listen → note song and `N of M` → Next if you want a non-first song → read **Do this now** / **Copy next step** (sit-down Next must match, or say no safe catalog next step) → read **Logic-ready next** / **Assets on file** / **Copy Logic-ready next** (Logic Pro's own words — Logic Pro project / WAV/AIFF / Stems / Recorded take / Key on file; no filename or path; no invented key or format) → Refresh (exact song must reopen with the same next step) → Escape / Clear filters (Resume strip still names the song and keeps Copy next step, Logic-ready next, and its Assets on file when safe) → **Resume** the same song → **Forget** (status: forgot; leftover work hash dropped; blocked storage soft-fails) → Refresh again (Resume stays gone). Extra fields, stale schema, unknown id, or a mismatched kind are rejected and removed. The latest Session Log remains authoritative.
