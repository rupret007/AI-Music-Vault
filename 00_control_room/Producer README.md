...
*How this system works, and how a future session resumes. Last updated: 2026-08-27 (usable catalog: local app_api.json after StoryBoard #12, not the spine, not a remote URL. Session Log #12 heading listed once. Resume from the latest Session Log H2, not Session 1. Local validate is the catalog gate; hosted empty-runner is not a catalog fail. Jeff-facing README/APPS.md are roles and counts only. Catalog-gate success output is roles and counts only).*
