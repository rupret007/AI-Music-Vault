...
*How this system works, and how a future session resumes. Last updated: 2026-08-27 (usable catalog: local app_api.json after StoryBoard #20, spine rejected as an import, not a remote URL. Show Night official set is owner-only. Official-set dump binds as Rad Dad — official set. Show Night is the live set surface. Catalog rows are not the official set. Show Night owns official sets. Session Log #12 heading listed once. Resume from the latest Session Log H2, not Session 1. Local validate is the catalog gate; hosted empty-runner is not a catalog fail. Jeff-facing README/APPS.md are roles and counts only. Catalog-gate success output is roles and counts only).*
