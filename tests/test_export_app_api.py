...
class ExportHonestyTests(unittest.TestCase):
    ...
    def test_bpm_field_is_what_storyboard_parses(self):
        ...
        self.assertIsNone(storyboard_parse_bpm(rec["bpm_raw"]))
    ...
    def test_field_map_matches_importer(self):
        ...
        self.assertEqual(mapping["field_map"]["bpm"], "bpm")
        self.assertEqual(mapping["field_map"]["musicalKey"], "key")
        self.assertNotEqual(mapping["field_map"]["active"], "is_original")
        self.assertNotEqual(mapping["field_map"]["notes"], "vault_ref")
        ...
        self.assertEqual(mapping["live_catalog_projects"], ["Rad Dad"])

    def test_default_live_setlist_is_empty_without_rad_dad_label(self):
        payload = build_payload(fixture())
    ...
    def test_check_fails_on_stale_field_map(self):
        cat = fixture()
        payload = build_payload(cat, generated="2026-08-23")
        payload["storyboard"]["field_map"]["bpm"] = "bpm_int"
        with tempfile.TemporaryDirectory() as tmp:
            path = os.path.join(tmp, "app_api.json")
            with open(path, "w", encoding="utf-8") as handle:
                json.dump(payload, handle)
