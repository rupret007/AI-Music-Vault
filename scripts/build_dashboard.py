...
function updateWorkSessionState(){
 ...
 if(workSessionEvidence){
  workSessionEvidence.textContent=!song
   ...
   :ev&&ev.n
    ?`${ev.n} searchable memo${ev.n===1?'':'s'}${ev.last?' · latest '+ev.last:''}. Review the evidence before working in Logic (export honesty, WAVs/AIFF/MIDI preference, no Ableton-first).`
